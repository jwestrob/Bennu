    ...
    async def ask(self, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Step 2: Validate preprocessing results (no fallback - fail fast if needed)
                detector_count = len(preprocess_bundle.detectors.get("functions", [])) + len(preprocess_bundle.detectors.get("domains", []))
                plan_count = len(preprocess_bundle.cypher_plans)
                
                logger.info(f"🔍 Schema-locked preprocessing results: {detector_count} detectors, {plan_count} plans")
                
                if detector_count == 0 and plan_count == 0:
                    logger.warning("⚠️ No detectors or plans found - proceeding with general query approach")
                    # Continue anyway - the agent can still perform whole_genome_reader or other tools
                ...
                # Generate plan with preprocessing bundle context (schema-locked approach)
                plan = await plan_initial_with_preprocess(question, settings, preprocess_bundle)
                ...
                logger.error(f"❌ Schema-locked execution failed: {e}")
                console.print(f"🚨 [red]Schema-locked execution error: {e}[/red]")
                # No fallback - raise the error to be handled by outer try-catch
                raise
...
async def plan_initial_with_preprocess(query: str, settings: Settings, preprocess_bundle: 'PreprocessBundle') -> Plan:
    """
    Generate initial execution plan with preprocessing bundle context.
    
    Args:
        query: User's natural language query
        settings: Settings for agent execution
    ...
    try:
        ...
        # Fallback to simple schema-locked plan
        return Plan(
            intent=Intent.FUNCTIONAL_ANALYSIS,
            steps=[
                PlanStep("database_query", cost="cheap", guard=Guard(name="always_true")),
                PlanStep("whole_genome_reader", cost="expensive", guard=Guard(name="requires_genomic_data"))
            ],
...
def _classify_query_intent(query: str, resolved_targets: Dict) -> Intent:
    ...
    Simplified schema-locked version.
    """
    query_lower = query.lower()
    
    # Spatial/neighborhood patterns (highest priority)
    spatial_patterns = [
        "find", "loci", "locus", "neighborhood", "spatial", "region", 
        "integrase", "prophage", "cluster", "around"
    ]
    if any(pattern in query_lower for pattern in spatial_patterns):
        return Intent.SPATIAL_NEIGHBORHOOD
    
    # Presence/absence patterns
    presence_patterns = ["exist", "present", "have", "contain"]
    if any(pattern in query_lower for pattern in presence_patterns):
        return Intent.PRESENCE_ABSENCE
    
    # Default to functional analysis
    return Intent.FUNCTIONAL_ANALYSIS


def _generate_plan_steps_with_preprocess(intent: Intent, resolved_targets: Dict, has_anchors: bool, 
                                       query: str, preprocess_bundle: 'PreprocessBundle') -> List[PlanStep]:
    """
    Generate plan steps that consume preprocessing bundle context.
    Schema-locked approach using cypher plans from preprocessing.
    """
    steps = []
    
    # Always start with database query that uses preprocessing cypher plans
    db_step = PlanStep(
        tool="database_query",
        cost="cheap",
        guard=Guard(name="always_true"),
        args={
            "preprocessing_bundle": preprocess_bundle,
            "query": query
        }
    )
    steps.append(db_step)
    
    # Add vector search for semantic matching if we have protein context
    if intent in [Intent.SPATIAL_NEIGHBORHOOD, Intent.NOVELTY_SCAN]:
        vector_step = PlanStep(
            tool="vector_search",
            cost="moderate",
            guard=Guard(name="requires_protein_data"),
            args={
                "similarity_threshold": 0.5,  # Default threshold
                "limit": 50,
                "exclude_terms": ["integrase", "recombinase"]  # For non-integrase searches
            }
        )
        steps.append(vector_step)
    
    # Add whole genome reader for spatial analysis
    if intent == Intent.SPATIAL_NEIGHBORHOOD and has_anchors:
        spatial_step = PlanStep(
            tool="whole_genome_reader",
            cost="expensive",
            guard=Guard(name="requires_genomic_data"),
            args={
