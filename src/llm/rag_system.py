...
from .annotation_tools import annotation_explorer, functional_classifier, annotation_selector, pathway_based_annotation_selector, assess_operon_context, prospect_operonic_proteins, enrich_proteins_with_context, comprehensive_protein_discovery
...
class ResultStreamer:
    ...
    def _create_session(self) -> str:
        """Create a new session directory and return session ID."""
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        self.session_id = f"session_{timestamp}_{uuid.uuid4().hex[:8]}"
        self.session_dir = self.output_dir / self.session_id
        self.session_dir.mkdir(parents=True, exist_ok=True)
        logger.info(f"Created streaming session: {self.session_id}")
        return self.session_id
    
    def stream_results(self, data_iterator: Iterator[Dict[str, Any]], 
                      session_id: str = None) -> List[str]:
        """
        Stream large result sets into chunked JSONL files.
        ...
        Returns:
            List of chunk summaries ("chunk 003: 587 rows")
        """
        if session_id:
        ...
        try:
            for record in data_iterator:
                # Convert record to JSON string for token counting
                record_json = json.dumps(record, default=str)
                record_tokens = self._count_tokens(record_json)
                
                # Check if adding this record would exceed chunk size
                if current_tokens + record_tokens > self.chunk_context_size and current_chunk:
                    # Write current chunk
                    chunk_file = self.session_dir / f"chunk_{chunk_num:03d}.jsonl"
                    self._write_chunk(chunk_file, current_chunk)
                    
                    # Create summary
                    summary = f"chunk {chunk_num:03d}: {len(current_chunk)} rows"
                    chunk_summaries.append(summary)
                    logger.info(f"Wrote {summary} to {chunk_file}")
            ...
            logger.info(f"Streaming complete: {total_rows} total rows in {len(chunk_summaries)} chunks")
            return chunk_summaries
            
        except Exception as e:
            logger.error(f"Error during result streaming: {e}")
            raise
    
    def _write_chunk(self, chunk_file: Path, chunk_data: List[Dict[str, Any]]):
    ...
    def get_session_info(self) -> Dict[str, Any]:
        ...
        if not self.session_dir or not self.session_dir.exists():
            return {"session_id": None, "chunks": 0, "total_size": 0}
...
AVAILABLE_TOOLS = {
    ...
    "enrich_proteins_with_context": enrich_proteins_with_context,
    "comprehensive_protein_discovery": comprehensive_protein_discovery,
}
...
class PlannerAgent(dspy.Signature):
    ...
    - requires_planning = true: Complex queries needing external tools or multi-step analysis
     ...
     - Operon discovery: "Find proteins in operons", "operonic central metabolism proteins"    WHEN TO USE TRADITIONAL MODE (requires_planning = false):
    ...
    - code_interpreter: Execute Python code for data analysis, visualization, and calculations
     ...
     - annotation_explorer: Catalog all functional annotations in the database
       Args: annotation_types=["KEGG", "PFAM"], functional_category="transport", max_annotations=1000
     - functional_classifier: Classify proteins by biological function (transport, metabolism, etc.)
       Args: annotation_catalog, functional_category="transport", user_preferences="", exclude_categories=[], max_relevant=10
     - annotation_selector: Select diverse examples from classified annotations
       Args: classified_annotations, selection_criteria="diverse", max_selected=5
     - operon_prospector: Find proteins in operons using LLM-powered assessment
       Args: functional_category="central_metabolism", min_examples=2, max_candidates=50
     - operon_assessor: Assess whether a protein occurs in a functionally coherent operon
       Args: protein_data, genomic_neighborhood, target_function="central_metabolism"
     - enrich_proteins_with_context: Add comprehensive genomic context to protein IDs
       Args: protein_ids=["list", "of", "protein", "ids"], max_proteins=50
     - comprehensive_protein_discovery: Complete workflow from functional category to enriched proteins
       Args: functional_category="central_metabolism", max_proteins=100, include_enrichment=true, batch_size=25     
     TASK TYPES:
         ...
         "tasks": [
             {
                 ...
                 "type": "tool_call",
                 "tool_name": "comprehensive_protein_discovery",
                 "tool_args": {"functional_category": "central_metabolism", "max_proteins": 150, "include_enrichment": true},
                 "dependencies": []
             },
             {
                 "id": "view_sequences",
                 "type": "tool_call",
                 "tool_name": "sequence_viewer",
                 "tool_args": {"protein_ids": "from comprehensive_discovery", "analysis_context": "central metabolism analysis"},
                 "dependencies": ["comprehensive_discovery"]
             },
             {
                 "id": "synthesize_results",
                 "type": "aggregate",
                 "dependencies": ["comprehensive_discovery", "view_sequences"]
             }
         ]
     }            }
        ]
...
class QueryClassifier(dspy.Signature):
    ...
    - BGC (biosynthetic gene cluster) information requests
...
class GenomicRAG(dspy.Module):
    ...
    def __init__(self, config: LLMConfig, chunk_context_size: int = 4096):
        ...
        # Initialize result streamer for large datasets
        self.result_streamer = ResultStreamer(chunk_context_size=chunk_context_size)
        ...
        logger.info(f"GenomicRAG system initialized with agentic capabilities and streaming (chunk size: {chunk_context_size})")
    ...
    async def _execute_traditional_query(self, question: str) -> Dict[str, Any]:
        ...
        context = await self._retrieve_context(classification.query_type, retrieval_plan, question)
    ...
    async def _execute_task(self, task: Task, previous_results: Dict[str, Any]) -> Any:
        ...
        if task.task_type == TaskType.ATOMIC_QUERY:
            ...
            context = await self._retrieve_context(classification.query_type, retrieval_plan, task.query)
            ...
            if task.tool_name == "sequence_viewer":
                ...
                # Log summary of tool_args to avoid dumping massive data structures
                tool_args_summary = {key: _safe_log_data(value, description=f'tool_arg_{key}') for key, value in task.tool_args.items()}
                logger.debug(f"🔬 Task tool_args summary: {tool_args_summary}")
                ...
                if needs_auto_population:
                    ...
                    logger.debug(f"🧬 Final protein IDs for sequence viewer: {_safe_log_data(unique_ids, description='final_protein_ids')}")
    ...
    def _enhance_code_interpreter(self, original_code: str, previous_results: Dict[str, Any]) -> str:
        ...
        for task_id, result in previous_results.items():
            # Extract from context (traditional query results)
            if "context" in result:
                context = result["context"]
                structured_data = context.structured_data
            ...
            # Extract from tool results (comprehensive discovery, enrichment, etc.)
            elif "tool_result" in result:
                tool_result = result["tool_result"]
                
                # From comprehensive_protein_discovery
                if result.get("tool_name") == "comprehensive_protein_discovery":
                    if tool_result.get("success") and "discovered_proteins" in tool_result:
                        for protein_data in tool_result["discovered_proteins"]:
                            pid = protein_data.get('protein_id', '')
                            if pid:
                                protein_ids.add(pid)
                
                # From enrich_proteins_with_context
                elif result.get("tool_name") == "enrich_proteins_with_context":
                    if tool_result.get("success") and "enriched_proteins" in tool_result:
                        for protein_data in tool_result["enriched_proteins"]:
                            pid = protein_data.get('protein_id', '')
                            if pid:
                                protein_ids.add(pid)
        ...
        if protein_ids:
            # Use all proteins if reasonable count, otherwise limit for performance
            max_proteins_for_code = 500  # Configurable limit for code interpreter
...
"""
        ...
        if protein_ids:
            ...
            logger.debug(f"Previous results structure: {_safe_log_data(list(previous_results.keys()), description='prev_results_keys')}")
            # Log what's actually in the previous results for debugging
            for task_id, result in previous_results.items():
                if isinstance(result, dict):
                    logger.debug(f"Task {task_id}: {_safe_log_data(list(result.keys()), description='task_result_keys')}")
                    if 'context' in result:
                        context = result['context']
                        if hasattr(context, 'structured_data'):
            ...
            if "context" in result:
                ...
                if tool_name == "sequence_viewer":
                    ...
                    if isinstance(tool_result, dict) and "formatted_display" in tool_result:
                        ...
                        tool_results.append(f"=== Protein Sequences: {task_id} ===\n{tool_result_summary}")
                elif tool_name == "comprehensive_protein_discovery":
                    # For comprehensive protein discovery, format the rich protein data properly
                    if isinstance(tool_result, dict) and tool_result.get("success") and "discovered_proteins" in tool_result:
                        proteins = tool_result["discovered_proteins"]
                        discovery_count = tool_result.get("discovery_count", len(proteins))
                        functional_category = tool_result.get("functional_category", "unknown")
                        
                        # Create a comprehensive summary with rich context
                        summary_lines = [
                            f"Comprehensive Protein Discovery Results: {discovery_count} {functional_category} proteins",
                            f"Functional Category: {functional_category}",
                            f"Enrichment Status: {'✅ Full genomic context included' if tool_result.get('enrichment_included') else '⚠️ Basic data only'}",
                            "",
                            "DISCOVERED PROTEINS WITH RICH GENOMIC CONTEXT:"
                        ]
                        
                        # Add detailed protein information (show all proteins)
                        for i, protein in enumerate(proteins):  # Show all discovered proteins with full context
                            protein_id = protein.get('protein_id', 'Unknown')
                            gene_start = protein.get('gene_start', 'N/A')
                            gene_end = protein.get('gene_end', 'N/A')
                            gene_strand = protein.get('gene_strand', 'N/A')
                            kegg_functions = protein.get('kegg_functions', [])
                            kegg_descriptions = protein.get('kegg_descriptions', [])
                            domain_descriptions = protein.get('domain_descriptions', [])
                            neighbor_count = protein.get('neighbor_count', 0)
                            
                            summary_lines.append(f"Protein {i+1}: {protein_id}")
                            summary_lines.append(f"  Coordinates: {gene_start}-{gene_end} bp ({gene_strand} strand)")
                            if kegg_functions:
                                summary_lines.append(f"  KEGG: {', '.join(kegg_functions[:3])} - {', '.join(kegg_descriptions[:3])}")
                            if domain_descriptions:
                                summary_lines.append(f"  Domains: {', '.join(domain_descriptions[:5])}")
                            if neighbor_count > 0:
                                summary_lines.append(f"  Genomic Context: {neighbor_count} neighbors within 5kb")
                            summary_lines.append("")
                        
                        # No truncation - show all proteins discovered
                        
                        tool_result_summary = "\\n".join(summary_lines)
                        tool_results.append(f"=== Comprehensive Discovery: {task_id} ===\\n{tool_result_summary}")
                    else:
                        tool_result_summary = str(tool_result)
                        tool_results.append(f"=== Discovery Tool: {tool_name} ({task_id}) ===\\n{tool_result_summary}")
    ...
    async def _retrieve_context(self, query_type: str, retrieval_plan, question: str = "") -> GenomicContext:
        ...
        try:
            ...
            if query_type == "general":
                # Check if this is a BGC-specific question
                if any(term in question.lower() for term in ['bgc', 'biosynthetic', 'cluster', 'secondary metabolite']):
                    # Use BGC-specific query
                    bgc_query = """
                    MATCH (genome:Genome)-[:HASBGC]->(bgc:Bgc)
                    OPTIONAL MATCH (bgc)<-[:PARTOFBGC]-(gene:Gene)<-[:ENCODEDBY]-(protein:Protein)
                    OPTIONAL MATCH (protein)-[:HASFUNCTION]->(ko:KEGGOrtholog)
    ...
    def _resolve_template_variables(self, tool_args: Dict[str, Any], previous_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in tool_args.items():
            ...
            if isinstance(value, str):
                ...
                # Pattern 1.5: Direct task name reference (e.g., "explore_annotations")
                    ...
                    task_ref = value
                    logger.info(f"🔧 Resolving direct task reference: {value} -> task_ref: {task_ref}")
                    
                    # For annotation_catalog, pass the entire tool result
                    if key == "annotation_catalog":
                        task_result = previous_results[task_ref]
                        if "tool_result" in task_result and task_result["tool_result"].get("success"):
                            resolved_value = task_result["tool_result"].get("annotation_catalog", {})
                            logger.info(f"🔧 Resolved annotation_catalog with {len(resolved_value)} categories")
                        else:
                            logger.warning(f"⚠️ No valid annotation_catalog found in task {task_ref}")
                            resolved_value = {}
                    else:
                        # Default behavior: extract protein IDs
                        protein_ids = self._extract_protein_ids_from_task(task_ref, previous_results)
                        resolved_value = protein_ids if protein_ids else []
                ...
                # Pattern 2: "<ids_from_task_name>" or "<task_name>"
                    ...
                    for pattern in template_patterns:
                        if pattern.startswith("ids_from_"):
                            ...
                            # For annotation_catalog, pass the entire tool result
                            if key == "annotation_catalog" and task_ref in previous_results:
                                task_result = previous_results[task_ref]
                                if "tool_result" in task_result and task_result["tool_result"].get("success"):
                                    resolved_value = task_result["tool_result"].get("annotation_catalog", {})
                                    logger.info(f"🔧 Resolved annotation_catalog with {len(resolved_value)} categories")
        ...
        # Log template resolution with safe data truncation
        original_summary = {key: _safe_log_data(value, description=f'original_{key}') for key, value in tool_args.items()}
        resolved_summary = {key: _safe_log_data(value, description=f'resolved_{key}') for key, value in resolved_args.items()}
        logger.info(f"🔧 Template resolution complete. Original keys: {list(tool_args.keys())} -> Resolved: {resolved_summary}")
    ...
    def _extract_protein_ids_from_task(self, task_ref: str, previous_results: Dict[str, Any]) -> List[str]:
        ...
        # Handle tool call results
            ...
            # Extract from comprehensive_protein_discovery
            if result.get("tool_name") == "comprehensive_protein_discovery":
                if tool_result.get("success") and "discovered_proteins" in tool_result:
                    for protein_data in tool_result["discovered_proteins"]:
                        pid = protein_data.get('protein_id', '')
                        if pid:
                            protein_ids.append(pid)
                    logger.info(f"🧬 Extracted {len(protein_ids)} proteins from comprehensive discovery")
            
            # Extract from enrich_proteins_with_context
            elif result.get("tool_name") == "enrich_proteins_with_context":
                if tool_result.get("success") and "enriched_proteins" in tool_result:
                    for protein_data in tool_result["enriched_proteins"]:
                        pid = protein_data.get('protein_id', '')
                        if pid:
                            protein_ids.append(pid)
                    logger.info(f"🧬 Extracted {len(protein_ids)} proteins from enrichment")
            
            # Extract from code interpreter output
            elif result.get("tool_name") == "code_interpreter":
    ...
    def _format_context(self, context: GenomicContext) -> str:
        ...
        if context.structured_data:
            ...
            is_bgc_query = any(
                'bgc.bgcId' in item or 'bgcId' in item 
                for item in context.structured_data
            )
            ...
            # Handle BGC-specific information
            if is_bgc_query:
                formatted_parts.append("\\nBIOSYNTHETIC GENE CLUSTER (BGC) ANALYSIS:")
                
                for item in context.structured_data:
                    genome_id = item.get('genome.genomeId', 'Unknown')
                    bgc_id = item.get('bgc.bgcId', 'Unknown')
                    bgc_product = item.get('bgc.bgcProduct', 'Unknown')
                    contig = item.get('bgc.contig', 'Unknown')
                    start_coord = item.get('bgc.startCoordinate', 'N/A')
                    end_coord = item.get('bgc.endCoordinate', 'N/A')
                    length_nt = item.get('bgc.lengthNt', 'N/A')
                    protein_count = item.get('bgc.proteinCount', 'N/A')
                    genes_in_bgc = item.get('genes_in_bgc', 0)
                    ko_functions = item.get('ko_functions', [])
                    
                    # GECCO probability scores
                    avg_prob = item.get('bgc.averageProbability', 'N/A')
                    max_prob = item.get('bgc.maxProbability', 'N/A')
                    alkaloid_prob = item.get('bgc.alkaloidProbability', 'N/A')
                    nrp_prob = item.get('bgc.nrpProbability', 'N/A')
                    polyketide_prob = item.get('bgc.polyketideProbability', 'N/A')
                    ripp_prob = item.get('bgc.rippProbability', 'N/A')
                    saccharide_prob = item.get('bgc.saccharideProbability', 'N/A')
                    terpene_prob = item.get('bgc.terpeneProbability', 'N/A')
                    domains = item.get('bgc.domains', 'N/A')
                    
                    formatted_parts.append(f"\\n• BGC: {bgc_id}")
                    formatted_parts.append(f"  - Genome: {genome_id}")
                    formatted_parts.append(f"  - Product Type: {bgc_product}")
                    formatted_parts.append(f"  - Location: {contig} ({_safe_format_int(start_coord)}-{_safe_format_int(end_coord)} bp)")
                    formatted_parts.append(f"  - Size: {_safe_format_int(length_nt)} nucleotides")
                    formatted_parts.append(f"  - Genes: {_safe_format_int(genes_in_bgc)} genes")
                    
                    # GECCO confidence scores
                    formatted_parts.append(f"  - GECCO Confidence:")
                    formatted_parts.append(f"    * Average: {_safe_format_float(avg_prob, precision=3)}")
                    formatted_parts.append(f"    * Maximum: {_safe_format_float(max_prob, precision=3)}")
                    
                    # Product-specific probabilities (only show high confidence ones)
                    product_probs = []
                    if _safe_format_float(alkaloid_prob, precision=3, default=0) != 0:
                        product_probs.append(f"Alkaloid: {_safe_format_float(alkaloid_prob, precision=3)}")
                    if _safe_format_float(nrp_prob, precision=3, default=0) != 0:
                        product_probs.append(f"NRP: {_safe_format_float(nrp_prob, precision=3)}")
                    if _safe_format_float(polyketide_prob, precision=3, default=0) != 0:
                        product_probs.append(f"Polyketide: {_safe_format_float(polyketide_prob, precision=3)}")
                    if _safe_format_float(ripp_prob, precision=3, default=0) != 0:
                        product_probs.append(f"RiPP: {_safe_format_float(ripp_prob, precision=3)}")
                    if _safe_format_float(saccharide_prob, precision=3, default=0) != 0:
                        product_probs.append(f"Saccharide: {_safe_format_float(saccharide_prob, precision=3)}")
                    if _safe_format_float(terpene_prob, precision=3, default=0) != 0:
                        product_probs.append(f"Terpene: {_safe_format_float(terpene_prob, precision=3)}")
                    
                    if product_probs:
                        formatted_parts.append(f"  - Product Probabilities: {', '.join(product_probs)}")
                    
                    # PFAM domains in BGC
                    if domains and domains != 'N/A':
                        domain_list = domains.split(';') if isinstance(domains, str) else domains
                        if domain_list and len(domain_list) > 0:
                            formatted_parts.append(f"  - Key Domains: {', '.join(domain_list[:10])}{'...' if len(domain_list) > 10 else ''}")
                    
                    if ko_functions and ko_functions != [None]:
                        # Filter out None values and show unique functions
                        valid_kos = [ko for ko in ko_functions if ko and ko != 'None']
                        if valid_kos:
                            formatted_parts.append(f"  - KEGG Functions: {', '.join(valid_kos[:8])}{'...' if len(valid_kos) > 8 else ''}")
