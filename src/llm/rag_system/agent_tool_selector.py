...
class IntelligentToolSelector:
    ...
    async def select_tool_for_task(self, 
        ...
        """
        logger.info(f"🎯 Selecting tool for task: {task_description[:100]}...")
        ...
        if self.agent_selector:
            try:
                ...
                if not agent_result.fallback_used:
                    ...
                    logger.info(f"🤖 Agent selected: {tool_name} (confidence: {agent_result.confidence:.2f})")
                    return agent_result
            except Exception as e:
                logger.error(f"💥 agent_fail: {type(e).__name__}: {e}")
                import traceback
                logger.debug(f"🐛 trace: {traceback.format_exc()}")
        
        # Fallback to regex-based selection
        logger.warning("🔄 fallback_to_regex")
    ...
    async def _agent_based_selection(self, 
        ...
        logger.debug(f"🔥 o3_call: task='{task_description[:50]}...'")
        ...
        logger.debug(f"🔥 o3_result: type={type(result)}, val={result}")
        
        if not result:
            logger.error("❌ o3_call: Model allocation returned None")
        ...
        logger.debug(f"🔍 o3_attrs: tool={has_selected}, args={has_args}, reason={has_reasoning}, decision={has_decision}")
        
        if has_selected:
            logger.debug(f"📤 o3_tool: '{result.selected_tool}'")
        if has_args:
            logger.debug(f"🛠️ o3_args: '{result.tool_arguments}'")
        if has_decision:
            logger.debug(f"🎯 o3_decision: '{result.use_this_tool}'")
        ...
        try:
            ...
            logger.debug(f"✅ parsed_args: {len(tool_args)} keys")
        except json.JSONDecodeError as e:
            logger.warning(f"❌ json_parse_fail: {e}")
            logger.warning(f"📝 raw_args: {result.tool_arguments}")
            tool_args = {"description": task_description}
        
        # Validate selected tool
        selected_tool = result.selected_tool
        logger.debug(f"🎯 raw_tool: '{selected_tool}'")
        if selected_tool not in AVAILABLE_TOOLS and selected_tool != "database_query":
            logger.warning(f"❌ invalid_tool: '{selected_tool}' not in {list(AVAILABLE_TOOLS.keys())}")
        ...
        logger.debug(f"🎯 use_decision: {use_tool}")
        ...
        if use_tool:
            ...
            logger.info(f"🔄 o3_says_NO: decision='{getattr(result, 'use_this_tool', 'MISSING')}'")
